...
def main():
    ...
    while True:
        ...
        # Camera capture #####################################################
        ret, image = cap.read()  # ret is a boolean value that returns true if the frame is available.
        if not ret:
            break
        ...
        # and each landmark is composed of x, y and z. x and y are normalized to [0.0, 1.0] by the image width
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                global last_hand_sign_id, hand_sign_id
...
def select_mode(key, mode):
    ...
    if key == 110:  # n
        mode = 0
    if key == 107:  # k - adds landmarks to dataset
        mode = 1
    return number, mode
